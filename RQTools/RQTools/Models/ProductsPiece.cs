namespace RQTools.Models
{
    using System;
    using System.Collections.Generic;
    using System.Text;
    public class ProductsPiece
    {
        public int Dializadores2_10 { get; set; }
        public int Dializadores1_90 { get; set; }
        public int Dializadores1_70 { get; set; }
        public int Dializadores1_50 { get; set; }
        public int Dializadores1_30 { get; set; }
        public int Dializadores1_10 { get; set; }
        public int Dializadores0_90 { get; set; }
        public int Dializadores0_70 { get; set; }
        public int Dializadores0_50 { get; set; }
        public int Dializadores0_30 { get; set; }
        public int LineasAdulto { get; set; }
        public int LineasPedriaticas { get; set; }
        public int KitCateter { get; set; }
        public int KitFistula { get; set; }
        public int Aquacid2k { get; set; }
        public int Aquacid0k { get; set; }
        public int Aquacid35CA { get; set; }
        public int Quabic { get; set; }
        public int Niprokart650 { get; set; }
        public int fistula15 { get; set; }
        public int fistula16 { get; set; }
        public int fistula17 { get; set; }
        public int Aquacetic { get; set; }
        public int Cloro { get; set; }
        public int Sal { get; set; }

    }
}
